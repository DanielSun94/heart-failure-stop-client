...
import {
    changeManagementQueryFilter, getDiagnosisInfo,
...
const DiagnosisPanel =({queryID}) =>{
    const classes = useStyles();
    const dispatch = useDispatch();
    const history = useHistory();
    const nextID = useSelector(state=>state.metaInfo.nextID);
    const diagnosisList = useSelector(state=>state.group.management[queryID].statistics.diagnosis.content);
    const isDataValid = useSelector(state=>state.group.management[queryID].statistics.diagnosis.isDataValid);
    ...
    const [diagnosisShowList, setDiagnosisShowList] = useState([]);
    ...
    useEffect(()=>{
        ...
        for(const item of diagnosisList){
            const firstLetterList = pinyin(diagnosisCodeMap[item.diagnosisCode], {style: pinyin.STYLE_FIRST_LETTER});
            let firstLetterStr = "";
            for(let strList of firstLetterList)
                firstLetterStr += strList[0];
            firstLetterStr = firstLetterStr.toLowerCase();
            pinyinMap[diagnosisCodeMap[item.diagnosisCode]]=firstLetterStr
        }
        cantUse4(pinyinMap);
    }, [diagnosisList, diagnosisCodeMap]);

    useEffect(()=>{
        // 此处filter>0的判断必须要有，因为如果删除一个查询，马上重建后，会直接触发服务器返回过时数据
        // 因此必须要重设filter后才能获取
        if((!isDataValid)&&(!isDataOutOfDate)&&Object.keys(filter).length>0){
            dispatch(getDiagnosisInfo(queryID))
        }
    ...
    useEffect(()=>{
        ...
        if(pinyinCode===''||pinyinCode==='输入拼音首字母过滤'){
            setDiagnosisShowList(diagnosisList)
        }
        else{
            for(const item of diagnosisList){
                const itemPinyin = namePinyinMap[diagnosisCodeMap[item.diagnosisCode]];
                if(itemPinyin&&itemPinyin.includes(pinyinLowCase)){
                    list.push(item)
                }
            }
            setDiagnosisShowList(list)
        }
        setPage(0)
    }, [pinyinCode, diagnosisList]);

    const handleClick=(diagnosisCode)=>{
        // 点击之后需要完成几个任务
        ...
        dispatch(editQueryName(fatherQueryNae+"_"+diagnosisCodeMap[diagnosisCode],
            false, nextID));
        ...
        newFilter[maxIdx+1]=[false, ParaName.DIAGNOSIS, diagnosisCode];
        dispatch(changeManagementQueryFilter(newFilter, nextID));

        // 4. 发起数据请求 此处构建新状态
        dispatch(queryDataAccordingToFilter(filter, nextID, queryID, [false, ParaName.DIAGNOSIS, diagnosisCode]));
        // 5. 实现跳转
    ...
    return (
        <Card className={classes.root}>
            <div className={classes.head}>
                <Typography variant={'h5'} style={{paddingLeft: 20, width: 150}}>出院诊断统计</Typography>
                <input
            ...
            <Paper className={classes.root}>
                <TableContainer className={classes.container}>
                    <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody>
                            {diagnosisShowList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                                return (
                                    <TableRow
                                    ...
                                    >
                                        <TableCell key={'name'} className={classes.name}>
                                            {diagnosisCodeMap[row.diagnosisCode]}
                                        </TableCell>
                ...
                <TablePagination
                    ...
                    component="div"
                    count={diagnosisShowList.length}
