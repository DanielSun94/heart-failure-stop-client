...
import {
    changeManagementQueryFilter, getMedicine,
...
const MedicinePanel =({queryID}) =>{
    const classes = useStyles();
    const dispatch = useDispatch();
    const history = useHistory();
    const nextID = useSelector(state=>state.metaInfo.nextID);
    const medicineList = useSelector(state=>state.group.management[queryID].statistics.medicine.content);
    const isDataValid = useSelector(state=>state.group.management[queryID].statistics.medicine.isDataValid);
    ...
    const medicineCodeMap = useSelector(state=>state.context.medicineCode.map);
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = React.useState(10);
    const [pinyinCode, setPinyin] = useState('输入拼音首字母过滤');
    const [medicineShowList, setMedicineShowList] = useState([]);
    ...
    useEffect(()=>{
        ...
        for(const item of medicineList){
            const firstLetterList = pinyin(medicineCodeMap[item.medicineCode], {style: pinyin.STYLE_FIRST_LETTER});
            let firstLetterStr = "";
            for(let strList of firstLetterList)
                firstLetterStr += strList[0];
            firstLetterStr = firstLetterStr.toLowerCase();
            pinyinMap[medicineCodeMap[item.medicineCode]]=firstLetterStr
        }
        cantUse4(pinyinMap);
    }, [medicineList, medicineCodeMap]);

    useEffect(()=>{
        // 此处filter>0的判断必须要有，因为如果删除一个查询，马上重建后，会直接触发服务器返回过时数据
        // 因此必须要重设filter后才能获取
        if((!isDataValid)&&(!isDataOutOfDate)&&Object.keys(filter).length>0){
            dispatch(getMedicine(queryID))
        }
    ...
    useEffect(()=>{
        ...
        if(pinyinCode===''||pinyinCode==='输入拼音首字母过滤'){
            setMedicineShowList(medicineList)
        }
        else{
            for(const item of medicineList){
                const itemPinyin = namePinyinMap[medicineCodeMap[item.medicineCode]];
                if(itemPinyin&&itemPinyin.includes(pinyinLowCase)){
                    list.push(item)
                }
            }
            setMedicineShowList(list)
        }
        setPage(0)
    }, [pinyinCode, medicineList]);

    const handleClick=(medicineCode)=>{
        // 点击之后需要完成几个任务
        ...
        dispatch(editQueryName(fatherQueryNae+"_"+medicineCodeMap[medicineCode],
            false, nextID));
        ...
        newFilter[maxIdx+1]=[false, ParaName.MEDICINE, medicineCode];
        dispatch(changeManagementQueryFilter(newFilter, nextID));

        // 4. 发起数据请求 此处构建新状态
        dispatch(queryDataAccordingToFilter(filter, nextID, queryID, [false, ParaName.MEDICINE, medicineCode]));
        // 5. 实现跳转
    ...
    return (
        <Card className={classes.root}>
            <div className={classes.head}>
                <Typography variant={'h5'} style={{paddingLeft: 20}}>药物统计</Typography>
                <input
            ...
            <Paper className={classes.root}>
                <TableContainer className={classes.container}>
                    <Table stickyHeader aria-label="sticky table">
                        <TableHead>
                            <TableRow>
                                <TableCell className={classes.name}>药物名称</TableCell>
                                <TableCell className={classes.count}>使用人次</TableCell>
                                <TableCell className={classes.ratio}>比例</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {medicineShowList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                                return (
                                    <TableRow
                                        id={row.medicineCode}
